$(function() {
    ...
    function() {
        ...
        (c = new THREE.PerspectiveCamera(75, d / h, 1, 1e4)).position.z = 1e3,
        p = new THREE.Scene,
        ...
        }), n = 0; n < 1e3; n++)(t = new THREE.Sprite(i)).position.x = 2 * Math.random() - 1,
        t.position.y = 2 * Math.random() - 1,
        t.position.z = 2 * Math.random() - 1,
        t.position.normalize(),
        t.position.multiplyScalar(10 * Math.random() + 450),
        t.scale.multiplyScalar(2),
        p.add(t);
        for (var n = 0; n < 300; n++) {
            var o = new THREE.Geometry,
            a = new THREE.Vector3(2 * Math.random() - 1, 2 * Math.random() - 1, 2 * Math.random() - 1);
            a.normalize(),
            a.multiplyScalar(450),
            o.vertices.push(a);
            var s = a.clone();
            s.multiplyScalar(.3 * Math.random() + 1),
            o.vertices.push(s);
    ...
    function e() {
        ...
        c.position.x += .05 * (t - c.position.x),
        c.position.y += .05 * (200 - r - c.position.y),
        c.lookAt(p.position),
        E.render(p, c)
    } ()
}),
...
THREE.CanvasRenderer = function(e) {
    ...
    e = e || {};
    var s, l, f, c, p, E, u, m, v, R, x, y, T, w, H, g, S, M, b, C = this,
    ...
    function ne(e, t, r) {
        ...
        if (K.min.set(e.x - o, e.y - o), K.max.set(e.x + o, e.y + o), r instanceof THREE.SpriteMaterial) {
            var a = r.map;
            if (null !== a) {
                var s = J[a.id];
            ...
            P.fillRect( - .5, -.5, 1, 1),
            P.restore()
        } else r instanceof THREE.SpriteCanvasMaterial && (ve(r.color.getStyle()), Re(r.color.getStyle()), P.save(), P.translate(e.x, e.y), 0 !== r.rotation && P.rotate(r.rotation), P.scale(i, n), r.program(P), P.restore())
    }
    function oe(e, t, r, i) {
        if (de(i.opacity), he(i.blending), P.beginPath(), P.moveTo(e.positionScreen.x, e.positionScreen.y), P.lineTo(t.positionScreen.x, t.positionScreen.y), i instanceof THREE.LineBasicMaterial) {
            if (fe(i.linewidth), ue(i.linecap), me(i.linejoin), i.vertexColors !== THREE.VertexColors) ve(i.color.getStyle());
            else {
        ...
        } else i instanceof THREE.LineDashedMaterial && (fe(i.linewidth), ue(i.linecap), me(i.linejoin), ve(i.color.getStyle()), xe([i.dashSize, i.gapSize]), P.stroke(), K.expandByScalar(2 * i.linewidth), xe([]))
    }
    function ae(e, t, r, i, n, o, a, s) {
        var l, c, p, E, d, h;
        if (C.info.render.vertices += 3, C.info.render.faces++, de(s.opacity), he(s.blending), u = e.positionScreen.x, m = e.positionScreen.y, v = t.positionScreen.x, R = t.positionScreen.y, x = r.positionScreen.x, y = r.positionScreen.y, l = u, c = m, p = v, E = R, d = x, h = y, P.beginPath(), P.moveTo(l, c), P.lineTo(p, E), P.lineTo(d, h), P.closePath(), (s instanceof THREE.MeshLambertMaterial || s instanceof THREE.MeshPhongMaterial) && null === s.map) U.copy(s.color),
        q.copy(s.emissive),
    ...
    function pe(e, t, r, i, n, o, a, s, l, c, p, E, d) {
        var h = J[d.id];
        if (void 0 !== h && h.version === d.version || (h = ce(d), J[d.id] = h), void 0 === h.canvas) return Re("rgba( 0, 0, 0, 1)"),
        void P.fill();
        Re(h.canvas);
        ...
        n -= e,
        o -= t,
        0 !== (y = (l -= a = (a + w) * g) * (E -= s = (s + H) * S) - (p -= a) * (c -= s)) && (R = e - (f = (E * r - c * n) * (T = 1 / y)) * a - (m = (l * n - p * r) * T) * s, x = t - (u = (E * i - c * o) * T) * a - (v = (l * o - p * i) * T) * s, P.save(), P.transform(f, u, m, v, R, x), P.fill(), P.restore())
    }
    function Ee(e, t, r) {
        var i, n = t.x - e.x,
        o = t.y - e.y,
    ...
    this.clear = function() { ! 1 === $.isEmpty() && ($.intersect(Y), $.expandByScalar(2), $.min.x = $.min.x + V, $.min.y = -$.min.y + j, $.max.x = $.max.x + V, $.max.y = -$.max.y + j, o < 1 && P.clearRect(0 | $.min.x, 0 | $.max.y, $.max.x - $.min.x | 0, $.min.y - $.max.y | 0), 0 < o && (he(THREE.NormalBlending), de(1), Re("rgba(" + Math.floor(255 * r.r) + "," + Math.floor(255 * r.g) + "," + Math.floor(255 * r.b) + "," + o + ")"), P.fillRect(0 | $.min.x, 0 | $.max.y, $.max.x - $.min.x | 0, $.min.y - $.max.y | 0)), $.makeEmpty())
    },
    ...
    this.render = function(e, t) {
        if (t instanceof THREE.Camera != !1) {
            var r = e.background;
            r && r.isColor ? (Re("rgb(" + Math.floor(255 * r.r) + "," + Math.floor(255 * r.g) + "," + Math.floor(255 * r.b) + ")"), P.fillRect(0, 0, h, z)) : !0 === this.autoClear && this.clear(),
            ...
            function() {
                ...
                t = f.length; e < t; e++) {
                    var r = f[e],
                    i = r.color;
                    r instanceof THREE.AmbientLight ? Q.add(i) : r instanceof THREE.DirectionalLight ? Z.add(i) : r instanceof THREE.PointLight && _.add(i)
                }
            } ();
            for (var i = 0,
            n = l.length; i < n; i++) {
...
THREE.Projector = function() {
    ...
    function() {
        ...
        return {
            ...
            pushTriangle: function(e, t, r) {
                ...
                n = le[t],
                o = le[r];
                if (!1 !== h(i, n, o) && (E.side === THREE.DoubleSide || !0 === f(i, n, o))) { (_ = Te()).id = p.id,
                    _.v1.copy(i),
                    _.v2.copy(n),
                    _.v3.copy(o),
    ...
    this.projectScene = function(e, t, r, i) {
        ...
        a = ce.objects.length; o < a; o++) {
            ...
            l = s.geometry;
            if (xe.setObject(s), oe = s.matrixWorld, Z = 0, s instanceof THREE.Mesh) {
                if (l instanceof THREE.BufferGeometry) {
                    var c = l.attributes,
                    p = l.groups;
                    if (void 0 === c.position) continue;
                    for (var E = 0,
                    d = (J = c.position.array).length; E < d; E += 3) xe.pushVertex(J[E], J[E + 1], J[E + 2]);
                    if (void 0 !== c.normal) {
                        var h = c.normal.array;
                        for (E = 0, d = h.length; E < d; E += 3) xe.pushNormal(h[E], h[E + 1], h[E + 2])
                    }
                    if (void 0 !== c.uv) {
                        var f = c.uv.array;
                        for (E = 0, d = f.length; E < d; E += 2) xe.pushUv(f[E], f[E + 1])
                    }
                    if (null !== l.index) {
                        var u = l.index.array;
                        if (0 < p.length) for (var m = 0; m < p.length; m++) {
                            var v = p[m];
                            for (E = v.start, d = v.start + v.count; E < d; E += 3) xe.pushTriangle(u[E], u[E + 1], u[E + 2])
                        } else for (E = 0, d = u.length; E < d; E += 3) xe.pushTriangle(u[E], u[E + 1], u[E + 2])
                    } else for (E = 0, d = J.length / 3; E < d; E += 3) xe.pushTriangle(E, E + 1, E + 2)
                    ...
                    H = !0 === w ? s.material: null, g = 0, S = R.length; g < S; g++) {
                        var M = R[g];
                ...
                if (l instanceof THREE.BufferGeometry) {
                    if (void 0 !== (c = l.attributes).position) {
                        var J;
                        for (E = 0, d = (J = c.position.array).length; E < d; E += 3) xe.pushVertex(J[E], J[E + 1], J[E + 2]);
                        if (null !== l.index) for (E = 0, d = (u = l.index.array).length; E < d; E += 2) xe.pushLine(u[E], u[E + 1]);
                        else {
                            var Y = s instanceof THREE.LineSegments ? 2 : 1;
                            for (E = 0, d = J.length / 3 - 1; E < d; E += Y) xe.pushLine(E, E + 1)
                        }
                    }
                } else if (l instanceof THREE.Geometry) {
                    if (fe.multiplyMatrices(he, oe), 0 === (R = s.geometry.vertices).length) continue; (k = ye()).positionScreen.copy(R[0]).applyMatrix4(fe);
                    for (Y = s instanceof THREE.LineSegments ? 2 : 1, g = 1, S = R.length; g < S; g++)(k = ye()).positionScreen.copy(R[g]).applyMatrix4(fe),
